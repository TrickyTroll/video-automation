...
def instruction_finder(read_path, filename):
    ...
    def shell_command(line):
        '''
        line: The line where the shell commands to execute start.
        This function returns a dictionnary that can be used by the shell 
        commands utility tool.
        '''
        dictionnay = {
                'command': line
        }
    ...
    with open(read_path / filename, "r") as f:
        ...
        for line in datafile:
            ...
            if '---' in line and '((instructions:' in datafile[index+1]:
                ...
                # if the user wants to make a callout, the structure should be
#                 similar to:
#                 to_add = {
#                         "command": name of slide,
#                         "media_format": callout,
#                         "file_name": [the string to callout]
#                         }
                todo.append(to_add)
...
def instructions_parser(instructions_list, supported_formats):
    '''
    This function takes the list of instructions that are in no 
    defined order and parses them in a dictionnary by type of media format.
    
    instructions_list: This is a list of dictionnaries generated by 
    instruction_finder.
    
    supported_formats: List of strings. These are the formats my functions 
    can do something with.
    
    returns: The instructions but in order.
    '''
    # The dictionnary should be generated in function a list of possible 
    # commands. This ensures that I wont have to change this function 
    # every week.
    dict_builder = [(key, [])]
    {key: value for (key, value) in supported_formats}
...
# ----------------------------------------------------------------------- #
#                       Code formatted text slides
# ----------------------------------------------------------------------- #
# The format will for now be defined as code.
# The reason behind this is the future gui.
# Since there is no file name, this could be replaced by customisation, like 
# telling wheter it's python code or shell code of wtv.
